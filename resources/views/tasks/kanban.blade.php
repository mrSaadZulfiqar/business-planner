              ...
              <button type="button" id="kanban-cancel-item" class="btn bg-gradient-light btn-sm pull-end me-2">Cancel</button>
                    ...
                    boards: [{
                        ...
                        item: [
                                ...
                                {
                                    id: "{{$task->id}}",
                                    title: '<small>Due Date: </small><span class="badge badge-sm bg-pink-light text-danger fw-bolder mb-1">@if(!empty($task->due_date)){{(\App\Supports\DateSupport::parse($task->due_date))->format(config('app.date_format'))}}@endif</span><h6 class="text fw-bolder mt-2">{{$task->subject}}</h6><p class="text mt-2">{{$task->description}}</p><div class="d-flex"><div class="avatar avatart-sm rounded-circle">@if(isset($users[$task->contact_id]))@if(!empty($users[$task->contact_id]->photo))<a href="javascript:" class="avatar avatar-sm rounded-circle"><img src="{{ asset('uploads/' . $users[$task->contact_id]->photo) }}"></a>@else<div class="avatar  avatar-sm rounded-circle bg-indigo"><p class=" mt-3 text-white text-uppercase">{{$users[$task->contact_id]->first_name[0]}}{{$users[$task->contact_id]->last_name[0]}}</p> </div> @endif</div><div class="text-sm fw-bold mt-2 ms-2">{{$users[$task->contact_id]->first_name}} {{$users[$task->contact_id]->last_name}} </div>@endif</div></div>',
                                    class: ["border-radius-md"]
                                },
                    ...
                    },
                        {
                            ...
                            item: [
                                ...
                                {
                                    id: "{{$task->id}}",
                                        title: '<small>Due Date: </small><span class="badge badge-sm bg-pink-light text-danger fw-bolder mb-1">@if(!empty($task->due_date)){{(\App\Supports\DateSupport::parse($task->due_date))->format(config('app.date_format'))}}@endif</span><h6 class="text fw-bolder mt-2">{{$task->subject}}</h6><p class="text mt-2">{{$task->description}}</p><div class="d-flex"><div class="avatar avatart-sm rounded-circle">@if(isset($users[$task->contact_id]))@if(!empty($users[$task->contact_id]->photo))<a href="javascript:" class="avatar avatar-sm rounded-circle"><img src="{{PUBLIC_DIR}}/uploads/{{$users[$task->contact_id]->photo}}"></a>@else<div class="avatar  avatar-sm rounded-circle bg-info"><p class=" mt-3 text-white text-uppercase">{{$users[$task->contact_id]->first_name[0]}}{{$users[$task->contact_id]->last_name[0]}}</p> </div> @endif</div><div class="text-sm fw-bold mt-2 ms-2">{{$users[$task->contact_id]->first_name}} {{$users[$task->contact_id]->last_name}} </div>@endif</div></div>',
                                    class: ["border-radius-md"]
                                },
                                @endforeach
                                @endif
                        ...
                        },
                        {
                            ...
                            item: [
                                ...
                                {
                                        id: "{{$task->id}}",
                                        title: '<small>Due Date: </small><span class="badge badge-sm bg-pink-light text-danger fw-bolder mb-1">@if(!empty($task->due_date)){{(\App\Supports\DateSupport::parse($task->due_date))->format(config('app.date_format'))}}@endif</span><h6 class="text fw-bolder mt-2">{{$task->subject}}</h6><p class="text mt-2">{{$task->description}}</p><div class="d-flex"><div class="avatar avatart-sm rounded-circle">@if(isset($users[$task->contact_id]))@if(!empty($users[$task->contact_id]->photo))<a href="javascript:" class="avatar avatar-sm rounded-circle"><img src="{{PUBLIC_DIR}}/uploads/{{$users[$task->contact_id]->photo}}"></a>@else<div class="avatar  avatar-sm rounded-circle bg-warning"><p class=" mt-3 text-white text-uppercase">{{$users[$task->contact_id]->first_name[0]}}{{$users[$task->contact_id]->last_name[0]}}</p> </div> @endif</div><div class="text-sm fw-bold mt-2 ms-2">{{$users[$task->contact_id]->first_name}} {{$users[$task->contact_id]->last_name}} </div>@endif</div></div>',
                                        class: ["border-radius-md"]
                                    },
                                @endforeach
                                    @endif
                        ...
                        },
                        {
                            ...
                            item: [
                                ...
                                {
                                        id: "{{$task->id}}",
                                        title: '<small>Due Date: </small><span class="badge badge-sm bg-pink-light text-danger fw-bolder mb-1">@if(!empty($task->due_date)){{(\App\Supports\DateSupport::parse($task->due_date))->format(config('app.date_format'))}}@endif</span><h6 class="text fw-bolder mt-2">{{$task->subject}}</h6><p class="text mt-2">{{$task->description}}</p><div class="d-flex"><div class="avatar avatart-sm rounded-circle">@if(isset($users[$task->contact_id]))@if(!empty($users[$task->contact_id]->photo))<a href="javascript:" class="avatar avatar-sm rounded-circle"><img src="{{PUBLIC_DIR}}/uploads/{{$users[$task->contact_id]->photo}}"></a>@else<div class="avatar  avatar-sm rounded-circle bg-success"><p class=" mt-3 text-white text-uppercase">{{$users[$task->contact_id]->first_name[0]}}{{$users[$task->contact_id]->last_name[0]}}</p> </div> @endif</div><div class="text-sm fw-bold mt-2 ms-2">{{$users[$task->contact_id]->first_name}} {{$users[$task->contact_id]->last_name}} </div>@endif</div></div>',
                                        class: ["border-radius-md"]
                                    },
                                @endforeach
                                    @endif
